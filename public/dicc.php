...
$query = "SELECT
    ...
    kcu.column_name,
    tc.constraint_type,
    pc.confrelid::regclass AS foreign_table_name
FROM
...
$result = pg_query($conn, $query);
$restricciones = [];
foreach (pg_fetch_all($result) as $r) {
    $rel = $r['constraint_type'] === 'FOREIGN KEY' ? ' ON ' . $r['foreign_table_name'] . '(id)' : '';
...
foreach ($tablas as $row) {
    ...
    $query_columnas = "SELECT column_name, data_type, dtd_identifier, character_maximum_length,
